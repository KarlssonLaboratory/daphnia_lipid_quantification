...
load_experiment <- function(experiment) {
  ...
  # food_treatment and chem_treatment.  We stack all replicates into one frame
...
# whether to include chem_treatment interaction and/or replicate offset.
...
#                 columns `food_treatment`, `chem_treatment`, and `resp`
...
make_histogram <- function(data, resp, experiment) {
  ...
  p_chem <- ggplot(df, aes(x = .data[[resp]], fill = chem_treatment)) +
    geom_histogram(
      aes(y = after_stat(density)),
      binwidth = bw, colour = "white", linewidth = 0.3, alpha = 0.8
    ) +
    geom_density(aes(colour = chem_treatment), linewidth = 0.7, show.legend = FALSE) +
    facet_wrap(~ chem_treatment, scales = "free_y") +
...
analyze_experiment <- function(data, responses = response) {
  ...
  p_drop <- ggplot(
    droplet_current,
    aes(x = food_treatment, y = area, fill = chem_treatment)
  ) +
    ...
    geom_text(
      data     = cld_drop_df,
      aes(x = food_treatment, y = y_max_drop, label = .group),
      hjust    = 0.5,
      vjust    = -0.5,
      position = position_dodge(width = 0.6),
      fontface = "bold",
